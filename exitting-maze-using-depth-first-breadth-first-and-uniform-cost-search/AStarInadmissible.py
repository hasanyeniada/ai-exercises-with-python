...
def getPossibleActionsAstar(expNode, maze, exploredNormal, pathCostUntilExploredNode, goal):
    ...
    for coor in actions:
        ...
        heuristic = giveInadmissibleHeuristic(coor, [goal[0], goal[1]])
        fN = heuristic + costOnTile
        coor.append(fN)
    return actions

def giveInadmissibleHeuristic(tile, goalTile): #inadmissible heuristic is given here.
    x1 = tile[0]
    y1 = tile[1]
    x2 = goalTile[0]
    y2 = goalTile[1]
    ...
    if (tile == [2,2]) or (tile == [3,2]) or (tile == [3,1]): #3 node's heuristics are now inadmissible and this will give wrong path
        heuristic = 10
    else:
        heuristic = math.ceil(dist)
    return heuristic
...
if __name__ == "__main__":
    ...
    solutionFlagAStar, exploredAStar = performAStarSearch(maze, startingTile, goalTile)  # maze, start point
    print("Resulting Tree:", exploredAStar)
    print("Tree is in that structure: [ [parent], [childs] ]")
    print("A* Search algorithm has finished...")

    if (solutionFlagAStar):
        print("Solution is found successfully with A* Search(Inadmissible)...")
        print("Resulting path:")
        resultingPath, pathLen = findResultingPath(exploredAStar, startingTile)
        result = resultingPath[::-1]
        print("Path cost according to fN: ", pathLen)
    else:
        print("No solution is found by A* Search(Inadmissible)...")
